...
string getRandomWordFixed(int x){
    return  mp[rand(0, x * uniqueWords/8 - 1)];
}

void genOneFile(int mb, int ff){
    const long long fileSize = 1LL * mb * 1024 * 1024;
    string temp = "file_" + to_string(ff) + ".txt";
    cout << temp << "\n";
    ...
    #pragma omp parallel num_threads(numThreads)
    {
        ...
        for (long long i = startPosition; i < endPosition; i += 10) {
            string word = getRandomWordFixed(ff + 1) + " ";
    ...
    fclose(file);
    cout << "Archivo de "<< fileSize/ (1024 * 1024) <<" MB generado exitosamente: " << filename << endl << endl;
}

int main() {
    readF();
    int mb = 100, numfiles = 8;
    for(int i = 0; i < numfiles; i++) genOneFile(100, i);    
    return 0;
}
